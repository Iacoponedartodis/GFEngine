...
- **Campi fantasma:** un campo dichiarato nello schema ma non consumato da nessun lettore →
  **Warning** automatico (rende KI #25 auto-diagnosticato invece che annotato a mano).
...
## Acceptance
- [ ] `GFEngine.exe --validate` esce ≠ 0 su un dato rotto ad arte (es. `aiProfileId` inesistente).
- [ ] Ogni Error ha categoria + file + suggerimento, ed è nel `session_latest.jsonl`.
- [ ] Rompere un riferimento **blocca** il load della missione invece di degradare.
- [ ] Il pannello editor mostra gli stessi errori del runtime (stesso codice).
- [ ] Un near-duplicate (KI #7) viene segnalato automaticamente.
